...
def binary():
	for r in range(rows):
		for c in range(columns):
			gray = 0.299*lenaCopy[r,c,2] + 0.587*lenaCopy[r,c,1] + 0.114*lenaCopy[r,c,0]
			if gray < 128:
				lenaCopy[r,c] = 0
			else:
				lenaCopy[r,c] = 255
...
def findCentroid(labelList, connectLabel):
	...
	recordBox = np.array([[rows,columns,0,0],[rows,columns,0,0],[rows,columns,0,0],[rows,columns,0,0],[rows,columns,0,0]])
	...
	for number in range(len(centerCal)):
		...
		cv2.circle(lenaCopy, (centerCal[number,1], centerCal[number,0]), 7, (28, 46, 184), -1)
		# Draw bounding box
		cv2.rectangle(lenaCopy, (recordBox[number][1],recordBox[number][0]), (recordBox[number][3],recordBox[number][2]), (247, 196, 27), 2)

	# print(centerCal)
	cv2.imshow("Circle", lenaCopy)
	cv2.waitKey(0)
	cv2.destroyAllWindows()
		...
		for c in range(columns):
			...
			if lenaCopy[r,c,0] == 255:
				...
				# First pixel
					...
					if lenaCopy[0,0,0] == 255:
						labels += 1
						connectLabel[r,c] = labels

				# Left-side pixels
				elif c == 0:
					if lenaCopy[r-1,c,0] == 255:
						connectLabel[r,c] = connectLabel[r-1,c]
					else:
						labels += 1
						connectLabel[r,c] = labels
